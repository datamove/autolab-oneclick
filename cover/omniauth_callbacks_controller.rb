  ...
  def github
    if user_signed_in?
      if data = request.env["omniauth.auth"]
        if User.find_for_slack_oauth(data).blank?
          current_user.authentications.create(provider: data["provider"], uid: data["uid"])
        end
      end
      redirect_to(root_path) && return
    else
      @user = CreateOdsUser.new(request.env["omniauth.auth"]).call
